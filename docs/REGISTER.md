...
| LL-011 | Look closely at one item | IDEA-003 | superseded | ++++ | [zoomItem](../src/canvas/zoomItem.ts) | mechanism replaced by LL-013 — pixels no longer cross the channel |
| LL-013 | Delegated vision over HTTP | LL-011 | built | ++++ | [zoomItem](../src/canvas/zoomItem.ts) | vision Q&A direct to Gemini server-side at FULL resolution; only the text answer enters the session; verified live: fine print read exactly, zero channel pixels, jitter flat |
